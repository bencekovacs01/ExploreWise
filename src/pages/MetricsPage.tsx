  ...
  const runPerformanceTests = async () => {
      ...
      try {
        ...
        try {
          if (algorithmType === 'Backtracking') {
            ...
            await backtrackingService.findMinimumRouteBt(coords, metadata);
            batchOutput += `Success\n`;
          } else if (algorithmType === 'BranchAndBound') {
            const BranchAndBoundService = (
              await import('../services/BranchAndBoundService')
            ).default;
            const branchAndBoundService = new BranchAndBoundService();
            ...
            const dynamicProgrammingService = new DynamicProgrammingService();
            ...
            const AcoService = (await import('../services/AcoService')).default;
            ...
            const GreedyService = (await import('../services/GreedyService'))
              .default;
            const greedyService = new GreedyService();
            batchOutput += `  Testing Greedy... `;
            await greedyService.findMinimumDistanceRoute(coords, metadata);
            batchOutput += `Success\n`;
    ...
    try {
      ...
      for (let i = 0; i < bitonicNodeCounts.length; i++) {
        const batchResults = await processBitonicBatch(bitonicNodeCounts[i]);
    ...
    <Box className="metrics-page" sx={{ p: 3 }}>
      ...
      <Paper elevation={3} sx={{ p: 3, mb: 4 }}>
        ...
        <Typography variant="body1" sx={{ mb: 2 }}>
          This will test algorithm performance using real POI data from
          poiData100.json with the following groups:
        </Typography>
        <Box component="ul" sx={{ mb: 2 }}>
          <Typography component="li">
            15 POIs: Branch-and-Bound, Dynamic Programming, ACO, Greedy, Arora
            PTAS, and Bitonic
          </Typography>
          <Typography component="li">
