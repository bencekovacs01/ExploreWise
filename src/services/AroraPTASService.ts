...
export class AroraPTASService {
  ...
  public getVisitDuration(
    ...
    if (
      ...
      const data = this.categoryDurations[categoryLower][subCategoryLower];
  ...
  private async findOptimalRouteInternal(
    ...
    if (pois.length < 2) {
      throw new Error('At least 2 POIs are required for PTAS optimization');
    }
  ...
  private async solveSmallInstance(
    ...
    if (poiMetadata) {
      for (let i = 1; i < route.length - 1; i++) {
        ...
        if (metadata) {
          visitTime +=
            this.getVisitDuration(metadata.category, metadata.subCategory) * 60;
        }
      }
    }
